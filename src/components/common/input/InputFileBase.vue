<template>
  ...
  <div class="attach_filewrap" v-if="!previewFlag">
    <label v-if="mode != modeDetail" class="btn_round2 btn_white btn_sm" :for="id"> 파일 추가 </label>
    <div class="attach_file_list">
      <div class="file_row" v-for="item in fileInfo" :key="item.fileUrl">
        <div class="file_name">
          <p>{{ item.fimOrgName }}</p>
          <i>{{item.size}} KB</i>
          <button type="button" class="btn_icon icon_only delete"  @click="removeFile(item.fileName)">삭제</button>
        </div>
      </div>
      ...
      <div class="file_row" v-for="(item,index) in fileUploadedInfo" :key="index">
        <div class="file_name">
          <p>{{ item.fimOrgName }}</p>
          <button type="button" class="btn_icon icon_only download" @click="dowload(item.fileName)">다운로드</button>
          <button type="button" class="btn_icon icon_only delete" @click="openModal(item)">삭제</button>
        </div>
      </div>
  ...
  methods: {
    ...
    handleFileChange(event) {
      ...
      if (
          ...
          (this.multiple && this.fileUploadedInfo.length + this.totalFile.length > this.maxFile)
      ) {
        return
      }
